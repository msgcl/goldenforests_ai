...
const COLORS = {
  ...
  warmHeader: rgb(0.663, 0.49, 0.325),
  warmHeaderDeep: rgb(0.557, 0.404, 0.267),
...
export async function downloadHomeOverviewPdf(siteCopy: SiteCopy, options: { heroImageUrl: string; logoUrl: string }) {
  ...
  const [logoBytes, heroBytes] = await Promise.all([
    fetchImageBytes(options.logoUrl),
    fetchImageBytes(options.heroImageUrl),
  ]);
  ...
  page.drawRectangle({
    ...
    width: PAGE_WIDTH,
    height: 122,
    color: COLORS.warmHeaderDeep,
    opacity: 0.32,
  });

  if (logoBytes) {
    try {
      const logo = options.logoUrl.toLowerCase().endsWith(".png")
      ...
      // Ignore asset errors and keep the export flowing.
    }
  }
  ...
  if (heroBytes) {
    try {
      const hero = options.heroImageUrl.toLowerCase().endsWith(".png")
        ? await pdfDoc.embedPng(heroBytes)
        : await pdfDoc.embedJpg(heroBytes);
  ...
  drawBulletList(
    ctx,
    copy.credibilityPartnerNames.map((name, index) => `${name}: ${copy.credibilityPartnerDescriptions[index] ?? ""}`),
    { size: 10.8 },
  );
  ...
  link.href = url;
  link.download = "golden-forests-home-overview.pdf";
